/*Todos los creditos al Programador David Mendoza Velis
   Nickname de youtube Deividcoptero
                   
                   Juego de Megaman se mueve con las flechas, se desliza con 
                   la barra de espacio, dispara con D brinca con S*/
...
void carregar(void){
 /******************        Aqui Definimos las imagenes que tendra cada variable       ******************/
    ...
    golpeb[0]  = load_bitmap("golpe_1.bmp",NULL);
    golpeb[1]  = load_bitmap("golpe_2.bmp",NULL);
    golpeb[2]  = load_bitmap("golpe_3.bmp",NULL);
    golpeb[3]  = load_bitmap("golpe_4.bmp",NULL);
    golpeb[4]  = load_bitmap("golpe_5.bmp",NULL);
    golpeb[5]  = load_bitmap("golpe_6.bmp",NULL);
...
// Funcion que muestra la animacion inicial (Megaman cayendo del cielo)
void Inicio_1(void){
    
     masked_blit(inicio_1,pBuffer,0,0,x,-30+i,inicio_1->w,inicio_1->h);
...
if(pos ==1){
     ...
     for(int i = 0;i<5;i++){
         blit(golpeb[i], screen , 0,0,x,y-35,80,125); 
         rest(120);
     ...
     for(int i = 0;i<5;i++){
        // blit(golpeb[i], screen , 0,0,x,y-35,80,125);
         draw_sprite_h_flip(golpeb[i],pBuffer,x,y-35); 
         rest(120);
         //blit(buffer, screen , 0,0,0,0,1300,450);
...
// Rutina para hacer todos los movimientos de megaman
void jugar(void){
    ...
    while(! key[KEY_ESC])
       {
               ...
               {
                   pintarbala();
                   blit(megaman_2,pBuffer,0,0,x,y,85,93);
...
int main(){	
	allegro_init();    
	install_keyboard();                                         //     Se inicializa el entorno
	
	set_color_depth(32);                                        //           de Allegro

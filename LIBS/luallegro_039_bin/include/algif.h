...
#define DAT_GIF DAT_ID('G', 'I', 'F', ' ')
	...
	int				algif_load_animation_pf(PACKFILE *file, BITMAP ***frames, int **durations);

	BITMAP			*load_gif(AL_CONST char *filename, RGB *pal);
	BITMAP			*load_gif_pf(PACKFILE *f, RGB *pal);

	int				save_gif(AL_CONST char *filename, BITMAP *bmp, AL_CONST RGB *pal);
	int				save_gif_pf(PACKFILE *file, BITMAP *bmp, AL_CONST PALETTE pal);

	/* Advanced use. */
	GIF_ANIMATION	*load_gif_object_pf(PACKFILE *file, long size);
	...
	int				algif_save_raw_animation_pf(PACKFILE *file, GIF_ANIMATION *gif);

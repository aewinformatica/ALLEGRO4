...
cvar_t *Cvar_Get (char *var_name, char *var_value, int flags)
{
	...
	var = (cvar_ptr)malloc (sizeof(*var));
...
cvar_t *Cvar_Set2 (char *var_name, char *value, bool force)
{
	...
	if (!force)
	{
		...
		if ((var->flags & CVAR_GAMELOCK) && 
			gamestate == GS_INGAME && 
			numtanks() >= 1)
		{
			Printf ("%s is cannot be changed while a game is in play.\n", var_name);
			return var;
		}
...
//
// Cvar_CheckForChanges - Checks for cvar changes and modifies the appropiate game settings
// (Eg. Used for fullscreen/windowed, music, sound volume...etc)
//
...
void	Cvar_CheckForChange(void)
{
	...
	// Check for fullscreen/windowed changes and do them here
	if (fullscreen->modified)
	{
		...
		// Screen resolution has been reset, reinstall the background
		// switch handlers
		InitSwitchState();
	}

	// Check for sound/music volume
	if (soundvolume->modified)
	{
		//voice_set_volume(Cvar_VariableValue("soundvolume"));
		soundvolume->modified = false;
	}

		// Check for sound/music volume
	if (musicvolume->modified)
	{
		SetMusicVolume();
		musicvolume->modified = false;
	}

	/*if (num_players->modified && teamplay->value > 0)
	{

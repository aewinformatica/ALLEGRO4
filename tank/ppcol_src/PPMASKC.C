...
PPCOL_MASK *create_ppcol_mask(BITMAP *sprite)
{
 ...
 maskout = (PPCOL_MASK *)malloc(sizeof(PPCOL_MASK) + (sizeof(unsigned long int *) * sprite->h) );
...
// (PPCOL_MASK *)maskout->sp_mask[0] = maskout->dat; //The first pointer points to the begining of the data...
 maskout->sp_mask[0] = (long unsigned int *) maskout->dat; //The first pointer points to the begining of the data...
